...
from conftest import build_sqlite_test_context
...
def test_alembic_upgrade_head_creates_core_tables(tmp_path: Path) -> None:
    ...
    engine, _ = build_sqlite_test_context(tmp_path, "alembic-smoke.db")
...
def test_alembic_upgrade_head_stamps_legacy_baseline_then_applies_followup_revisions(
    ...
    database_url = f"sqlite+aiosqlite:///{db_file.as_posix()}"
    engine, _ = build_sqlite_test_context(tmp_path, "alembic-legacy-baseline.db")
